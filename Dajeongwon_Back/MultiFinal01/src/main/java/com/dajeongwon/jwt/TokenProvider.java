...
public class TokenProvider  {
    ...
    public TokenDto generateTokenDto(Authentication authentication) {
        ...
        // AccessToken 생성 
        Date tokenExpiresIn = new Date(now + ACCESS_TOKEN_EXPIRE_TIME);		// 권한 만료되는 날짜 (now +  30분)
        
        System.err.println(authorities+"!!!!!!!!!!테스트!!!!!!!!!!");
        System.out.println("authentication" + authentication.getName());
        String accessToken = Jwts.builder()
                .setSubject(authentication.getName())	// sub : name
                .claim(AUTHORITIES_KEY, authorities)	// auth : ROLE_USER
                .setExpiration(tokenExpiresIn)			// exp : 만료 날짜인 듯 
                .signWith(key, SignatureAlgorithm.HS512) // header "alg": "HS512"
                .compact();
        

        // 리프레시 토큰도 생성해주는 게 좋은데 힘들어서 생략함 
