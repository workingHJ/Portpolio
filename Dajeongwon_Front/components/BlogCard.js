    ...
    <div {...props} className={wrapperClass}>
      ...
      <div className={cardBodyClass}>
        {category && (
          <>
            {category.href ? (
              <Link href={category.href} className={`${size === "lg" ? "fs-sm" : "fs-xs"} text-uppercase text-decoration-none`}>
                {category.title}
              </Link>
            ) : (
              <div className={`${size === "lg" ? "fs-sm" : "fs-xs"} text-uppercase text-primary`}>{category.title}</div>
            )}
          </>
        )}
        {title && (
          <h3 className={`${size === "lg" ? "h5" : "fs-base"} pt-1 mb-2`}>
            {href ? (
        ...
        {type !== "card" && (
          <>
            {author ? (
              <>
                {author.href ? (
                  <Link href={author.href} className="d-flex align-items-center text-decoration-none pt-2">
                    {author.img && (
                      <div className="position-relative flex-shrink-0 rounded-circle overflow-hidden me-2" style={size === "lg" ? {width: "48px", height: "48px"} : {width: "44px", height: "44px"}}>
                        <ImageLoader src={author.img} layout="fill" objectFit="cover" quality={90} alt={author.name} light={light ? 1 : 0} />
                      </div>
                    )}
                    <div>
      ...
      {type === "card" && (
        <>
          {author ? (
            <>
              {author.href ? (
                <Link href={author.href} className="card-footer d-flex align-items-center text-decoration-none border-top-0 pt-2 mb-1">
                  {author.img && (
                    <div className="position-relative flex-shrink-0 rounded-circle overflow-hidden me-2" style={size === "lg" ? {width: "48px", height: "48px"} : {width: "44px", height: "44px"}}>
                      <ImageLoader src={author.img} layout="fill" objectFit="cover" quality={90} alt={author.name} light={light ? 1 : 0} />
                    </div>
                ...
                <div className="card-footer d-flex align-items-center text-decoration-none border-top-0 pt-2 mb-1">
                  {author.img && (
                    <div className="position-relative flex-shrink-0 rounded-circle overflow-hidden me-2" style={size === "lg" ? {width: "48px", height: "48px"} : {width: "44px", height: "44px"}}>
                      <ImageLoader src={author.img} layout="fill" objectFit="cover" quality={90} alt={author.name} light={light ? 1 : 0} />
                    </div>
