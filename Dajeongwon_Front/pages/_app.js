...
const Finder = ({Component, pageProps}) => {
  ...
  const decodeHTMLString = (htmlString) => {
    ...
    const decodedString = parser.parseFromString(`<!doctype html><body>${htmlString}`, "text/html").body.textContent;
  ...
  return (
    <SSRProvider>
      ...
      <LoginContext.Provider value={{loginInfo, setLoginInfo, decodeHTMLString}}>
        <Component {...pageProps} />
      </LoginContext.Provider>

      <ScrollTopButton showOffset={600} duration={800} easing="easeInOutQuart" tooltip="Top" />
    </SSRProvider>
  );
};
