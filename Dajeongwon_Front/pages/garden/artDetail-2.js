...
const GardenArt = ({gNo, aNo}) => {
  ...
  const [flowerStatus, setFlowerStatus] = useState(1);
  const [flower, setFlower] = useState("");
  const router = useRouter();
  const {accessType} = router.query;
  useEffect(() => {
    getArtList(gNo);
    getArtData(gNo, aNo);
    if (artData) {
  ...
  const getArtData = async (gNo, aNo) => {
    try {
      ...
      if (artResponse.data.result === true) {
        setArtData(artResponse.data.artData);
        setMeetingDate(artResponse.data.meetingDate);
        setGardenTitle(artResponse.data.gardenTitle);
        setFlowerStatus(artResponse.data.flowerStatus);
        setFlower(artResponse.data.flower);
      }
            ...
            <Offcanvas show={show} onHide={handleClose} placement="end" responsive="lg">
              ...
              <Offcanvas.Body>
                ...
                <Card.Body className="text-center">
                  ...
                  <h4 className="hanamdaum pb-0 mb-1">{artData.title}</h4>
                  <StarRating rating={artData.rating}></StarRating>
                  <span className="text-beige ms-1 fs-sm">({artData.reviewCount})</span>
                </Card.Body>

                {/* 다봤어요 버튼 */}
                <Button size="lg" variant={status === "COMPLETE" ? "primary" : "translucent-primary"} className="w-100 mb-3 mt-4" style={{left: 0, right: 0, border: 0}} onClick={handleStatusChange}>
...
//     if (reviewResponse.data.result == true) {
//       reviewList = reviewResponse.data.artReviewList;
//     } else {
//       throw new Error("데이터를 가져오는 데 실패함");
//     }
