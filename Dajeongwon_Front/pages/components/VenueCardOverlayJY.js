...
const VenueCardOverlayJY = ({img, title, date, time, button, overlay, className, ...props}) => {
  return (
            ...
            <div className="btn-group">
              {button.href ? (
                <Link
                  href={button.href}
                  {...button.props}
                  className={button.variant ? `btn btn-${button.variant} rounded-end-0 px-3 pyeongChang-bold` : "btn btn-primary rounded-end-0 px-3 pyeongChang-bold"}
                >
                  {button.title}
                </Link>
              ) : (
                <button type="button" className={button.variant ? `btn btn-${button.variant} rounded-end-0 px-3 pyeongChang-bold` : "btn btn-primary rounded-end-0 px-3 pyeongChang-bold"}>
                  {button.title}
                </button>
              )}
              <div className="posiion-relative border-start border-light zindex-5" style={{marginLeft: "-1px"}}></div>
              <button {...button.wishlistProps} type="button" className={button.variant ? `btn btn-${button.variant} rounded-start-0 px-3` : "btn btn-primary rounded-start-0 px-3"}>
                <i className="fi-heart-filled"></i>
                {/* 하트 안채워진거 : fi-heart */}
                {/* 하트 채워진거 : fi-heart-filled */}
              </button>
            </div>
          )}
        </div>
...
export default VenueCardOverlayJY;
